...
class	Automemcache	extends	CI_Controller
{
				...
				public	function	set_instantiation_facet()
				{
								...
								$this->memcached_library->add('ins_state',	json_encode(sortByOneKey($states['records'],	'state')));
								$this->myLog('State Count '.count($this->memcached_library->get('ins_state')));
								
								$this->memcached_library->add('ins_stations',	json_encode(sortByOneKey($stations['records'],	'organization')));
								$this->myLog('Station Count '.count($this->memcached_library->get('ins_stations')));
								
								$this->memcached_library->add('ins_status',	json_encode(sortByOneKey($nomination['records'],	'status')));
								$this->myLog('Status Count '.count($this->memcached_library->get('ins_status')));
								
								$this->memcached_library->add('ins_media_type',	json_encode(sortByOneKey($media_type['records'],	'media_type',	TRUE)));
								$this->myLog('Media Type Count '.count($this->memcached_library->get('ins_media_type')));
								
								$this->memcached_library->add('ins_physical',	json_encode(sortByOneKey($p_format['records'],	'format_name',	TRUE)));
								$this->myLog('Physical Format Count '.count($this->memcached_library->get('ins_physical')));
								
								$this->memcached_library->add('ins_digital',	json_encode(sortByOneKey($d_format['records'],	'format_name',	TRUE)));
								$this->myLog('Digital Format Count '.count($this->memcached_library->get('ins_digital')));
								
								$this->memcached_library->add('ins_generations',	json_encode(sortByOneKey($generation['records'],	'facet_generation',	TRUE)));
								$this->myLog('Generation Count '.count($this->memcached_library->get('ins_generations')));
								
								$this->memcached_library->add('ins_digitized',	json_encode($digitized));
								$this->myLog('Digitized Count '.count($this->memcached_library->get('ins_digitized')));
								
								$this->memcached_library->add('ins_migration',	json_encode($migration));
