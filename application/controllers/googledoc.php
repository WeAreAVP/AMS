...
class Googledoc extends MY_Controller
{
    ...
    {
	if ((isset($row[8]) && !empty($row[8])) or (isset($row[9]) && !empty($row[9])))
	{
	    $event_data = array();
    ...
    {
	if ((isset($row[12]) && !empty($row[12])) or (isset($row[13]) && !empty($row[13])))
	{
	    $event_type = 'baked';
	    $event_data['instantiations_id'] = $instantiation_id;
	    $event_type_data = $this->instantiation->get_id_by_event_type($event_type);
    ...
    {
	if ((isset($row[14]) && !empty($row[14])) or (isset($row[16]) && !empty($row[16])))
	{
	    $event_type = 'cleaned';
	    $event_data['instantiations_id'] = $instantiation_id;
	    $event_type_data = $this->instantiation->get_id_by_event_type($event_type);
    ...
    {
	if ((isset($row[17]) && !empty($row[17])) or (isset($row[34]) && !empty($row[34])) or (isset($row[35]) && !empty($row[35])))
	{
