...
class Dompdf_lib
{
	...
	function convert_html_to_pdf($data, $filename = 'sample.pdf', $stream = TRUE)
	{
		...
		if (count($data['dsd_report']) > 0)
		{
			...
			$html .='<tr><td><b>Total</b><p style="text-align:right;float:left;">' . number_format(count($data['dsd_report'])) . '</p></td><td style="border-top:1px solid black;">' . number_format($total) . '</td><td style="border-top:1px solid black;"></td><td style="border-top:1px solid black;"></td></tr>';
		...
		if (count($data['material_at_crawford_report']) > 0)
		{
			...
			$html .='<tr><td><b>Total</b><span style="text-align:right;">' . number_format(count($data['material_at_crawford_report'])) . '</span></td><td style="border-top:1px solid black;">' . number_format($total) . '</td><td style="border-top:1px solid black;"></td><td style="border-top:1px solid black;"></td></tr>';
			$html .='</tbody></table></div>';
		}
