	...
	function get_users($role = null, $params = null)
	{
		...
		if ($role != null)
		{
			if ($role == 1 || $role == 20)
			{
				...
				$this->db->where_not_in("$users_table.role_id", array('1', '2', '5'));
			else if ($role == 5)
				$this->db->where_not_in("$users_table.role_id", array('1', '2', '3', '4'));
			else
		...
		{
			$this->db->where("$users_table.station_id", $this->station_id);
		}
		$this->db->order_by("$users_table.id", "ASC");
		$query = $this->db->get($this->_table);
		return $query;
