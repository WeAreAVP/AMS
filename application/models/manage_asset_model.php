...
class Manage_Asset_Model extends CI_Model
{
	...
	function get_asset_detail_by_id($asset_id)
	{
		...
		$this->db->select("GROUP_CONCAT(DISTINCT asset_types.asset_type  SEPARATOR ' | ') AS asset_type");
		$this->db->select("GROUP_CONCAT(DISTINCT asset_dates.asset_date  SEPARATOR ' | ') AS asset_date");
		$this->db->select("GROUP_CONCAT(DISTINCT date_types.date_type  SEPARATOR ' | ') AS date_type");
