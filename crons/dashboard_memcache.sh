...
HOST=`/bin/hostname`
PID_FILE="PIDs/dashboard_memcache.pid"
OUTPUT_FILE="cronlog/dashboard_memcache.log"
...
CMD="$PHP_PATH ${BASEDIR}index.php crons dashboard_memcached > $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH"
MSG="ENV [$ENVIRONMENT]"
...
$PHP_PATH ${BASEDIR}index.php crons dashboard_memcached > $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH
PID=`cat $PID_FULLPATH`
