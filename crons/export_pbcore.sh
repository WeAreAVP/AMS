...
HOST=`/bin/hostname`
PID_FILE="PIDs/export_pbcore.pid"
OUTPUT_FILE="cronlog/export_pbcore.log"
...
CMD="$PHP_PATH ${BASEDIR}index.php xml export_pbcore > $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH"
MSG="ENV [$ENVIRONMENT]"
...
$PHP_PATH ${BASEDIR}index.php xml export_pbcore >> $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH
PID=`cat $PID_FULLPATH`
