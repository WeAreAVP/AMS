...
CMD="$PHP_PATH ${BASEDIR}index.php crons make_refine_csv >> $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH"
MSG="ENV [$ENVIRONMENT]"
...
$PHP_PATH ${BASEDIR}index.php crons make_refine_csv >> $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH
PID=`cat $PID_FULLPATH`
