...
HOST=`/bin/hostname`
PID_FILE="PIDs/mediainfo_import_xml.pid"
OUTPUT_FILE="cronlog/mediainfo_import_xml.log"
...
CMD="$PHP_PATH ${BASEDIR}index.php mediainfo process_xml_file > $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH"
MSG="ENV [$ENVIRONMENT]"
...
$PHP_PATH ${BASEDIR}index.php mediainfo process_xml_file > $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH
PID=`cat $PID_FULLPATH`
